"""
Scraper for Hong Kong Baptist University (HKBU) master's programs.

Official source: https://gs.hkbu.edu.hk/admission/taught-postgraduate-programmes
"""
...
class HKBUScraper(BaseUniversityScraper):
    """Scraper for HKBU taught postgraduate programmes."""

    def __init__(self):
        super().__init__(HK_UNIVERSITIES["hkbu"])

    def scrape_programs(self) -> List[ProgramInfo]:
        """
        Scrape HKBU's taught postgraduate programme list.

        Official URL: https://gs.hkbu.edu.hk/admission/taught-postgraduate-programmes
        """
        programs = []
        ...
        if not soup:
            logger.error("[HKBU] Failed to fetch programme list page")
            return self._fallback_programs()

        items = soup.select(
            "table tbody tr, .programme-item, .views-row, "
            ".program-card, .list-item, .accordion-body a"
        )
    ...
    def _parse_item(self, item, base_url: str) -> Optional[ProgramInfo]:
        try:
            ...
            logger.debug(f"[HKBU] Error parsing item: {e}")
            return None
        ...
        return [
            self.create_program(
                ...
                remarks=(
                    ...
                    "HKBU Graduate School page for the full programme list."
                ),
            )
        ]
