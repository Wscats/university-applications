"""
Scraper for Lingnan University (LU) master's programs.

Official source: https://www.ln.edu.hk/admissions/postgraduate/taught-postgraduate-programmes
"""
...
class LingnanScraper(BaseUniversityScraper):
    """Scraper for Lingnan University postgraduate programmes."""

    def __init__(self):
        super().__init__(HK_UNIVERSITIES["lingnan"])

    def scrape_programs(self) -> List[ProgramInfo]:
        """
        Scrape Lingnan's postgraduate programme list.

        Official URL: https://www.ln.edu.hk/admissions/postgraduate/taught-postgraduate-programmes
        """
        programs = []
        ...
        if not soup:
            logger.error("[LU] Failed to fetch programme list page")
    ...
    def _parse_item(self, item, base_url: str) -> Optional[ProgramInfo]:
        try:
            ...
            logger.debug(f"[LU] Error parsing item: {e}")
            return None
        ...
        return [
            self.create_program(
                ...
                remarks=(
                    ...
                    "Lingnan University admissions page for the full programme list."
                ),
            )
        ]
